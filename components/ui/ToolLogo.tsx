...
export default function ToolLogo({ slug, domain, name, size = 48 }: ToolLogoProps) {
  ...
  if (hasCustomIcon) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-white/4 border border-white/8 rounded-xl">
        <ToolIcon slug={slug} size={iconSize} className="text-[#C8C8C8]" />
      </div>
    )
  }
  ...
  if (!domain || clearbitFailed) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-neon/10 border border-neon/20 rounded-xl">
        <span
          className="text-neon font-black leading-none select-none"
  ...
  return (
    <div className="w-full h-full flex items-center justify-center bg-white/5 border border-white/8 rounded-xl overflow-hidden">
      <Image
